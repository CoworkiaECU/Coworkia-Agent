  ...
  const photoGrid = photoAssets.length > 0
    ? `<div style="margin-bottom:28px;">
        ...
        <div style="display:grid;grid-template-columns:repeat(${Math.min(photoAssets.length, 4)},1fr);gap:6px;">
          ${photoAssets.map((_, i) => `<img src="cid:foto-${i+1}@paintbull" alt="Foto ${i+1}" style="width:100%;height:110px;object-fit:cover;border-radius:6px;border:1px solid #E5E7EB;display:block;" />`).join('')}
        </div>
      </div>`
    : '';
